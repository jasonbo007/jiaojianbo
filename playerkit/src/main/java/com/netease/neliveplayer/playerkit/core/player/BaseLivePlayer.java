...
//                player.reset();
    ...
    private void switchContentUrlWithDecryptionToken(String newVideoPath, String transferToken, String accid, String token, String appKey) {
        synchronized (lock) {
            if (player != null) {
                if (TextUtils.isEmpty(newVideoPath)) {
                    return;
    ...
    private void switchContentUrlWithDecryptionKey(String newVideoPath, byte[] flvKey, int flvKeyLen) {
        synchronized (lock) {
            if (player != null) {
                if (TextUtils.isEmpty(newVideoPath)) {
                    return;
    ...
    public void switchContentUrl(String newVideoPath, DataSourceConfig config) {
        ...
        if(options.isUseNewDecryptionMethod) {
            ...
            if (config.decryptionConfig.decryptionCode == DecryptionConfigCode.CODE_DECRYPTION_INFO) {
                switchContentUrlWithDecryptionToken(newVideoPath,config.decryptionConfig.transferToken, config.decryptionConfig.accid, config.decryptionConfig.token, config.decryptionConfig.appKey);
            }else if (config.decryptionConfig.decryptionCode == DecryptionConfigCode.CODE_DECRYPTION_KEY) {
                switchContentUrlWithDecryptionKey(newVideoPath, config.decryptionConfig.flvKey, config.decryptionConfig.flvKeyLen);
            }else {
        ...
        synchronized (lock) {
            if (player != null) {
                ...
                if(config.cacheConfig!=null) {
                    dataSourceConfig.cacheConfig = new NECacheConfig(config.cacheConfig.isCache, config.cacheConfig.cachePath);
                }
                if (config.decryptionConfig != null) {
                    if (config.decryptionConfig.decryptionCode == DecryptionConfigCode.CODE_DECRYPTION_INFO) {
                        dataSourceConfig.decryptionConfig = new NEDecryptionConfig(config.decryptionConfig.transferToken, config.decryptionConfig.accid, config.decryptionConfig.appKey, config.decryptionConfig.token);
                    }else if (config.decryptionConfig.decryptionCode == DecryptionConfigCode.CODE_DECRYPTION_KEY) {
                        dataSourceConfig.decryptionConfig = new NEDecryptionConfig(config.decryptionConfig.flvKey, config.decryptionConfig.flvKeyLen);
                    }else {
                        LogUtil.error(" player need init dataSourceConfig");
                    }
    ...
    private void initPlayer() {
        synchronized (lock) {
            ...
            if(!options.isUseNewDecryptionMethod && options.dataSourceConfig!=null && options.dataSourceConfig.decryptionConfig!=null) {
                if (options.dataSourceConfig.decryptionConfig.decryptionCode == DecryptionConfigCode.CODE_DECRYPTION_INFO) {
                    player.prepareAsyncWithDecryptionToken(options.dataSourceConfig.decryptionConfig.transferToken, options.dataSourceConfig.decryptionConfig.accid, options.dataSourceConfig.decryptionConfig.token, options.dataSourceConfig.decryptionConfig.appKey,onDecryptionListener);
                }else if (options.dataSourceConfig.decryptionConfig.decryptionCode == DecryptionConfigCode.CODE_DECRYPTION_KEY) {
                    player.prepareAsyncWithDecryptionKey(options.dataSourceConfig.decryptionConfig.flvKey, options.dataSourceConfig.decryptionConfig.flvKeyLen,onDecryptionListener);
                }else {
                    LogUtil.error("initPlayer player need init dataSourceConfig");
                }
            }else {
                player.prepareAsync();
            }
    ...
    private void configPlayer() {
        ...
        try {
            ...
            } else if (options.isUseNewDecryptionMethod && options.dataSourceConfig != null) {
                NEDataSourceConfig dataSourceConfig = new NEDataSourceConfig();
                if(options.dataSourceConfig.cacheConfig!=null) {
                    dataSourceConfig.cacheConfig = new NECacheConfig(options.dataSourceConfig.cacheConfig.isCache, options.dataSourceConfig.cacheConfig.cachePath);
                }
                if (options.dataSourceConfig.decryptionConfig != null) {
                    if (options.dataSourceConfig.decryptionConfig.decryptionCode == DecryptionConfigCode.CODE_DECRYPTION_INFO) {
                        dataSourceConfig.decryptionConfig = new NEDecryptionConfig(options.dataSourceConfig.decryptionConfig.transferToken, options.dataSourceConfig.decryptionConfig.accid, options.dataSourceConfig.decryptionConfig.appKey, options.dataSourceConfig.decryptionConfig.token);
                    }else if (options.dataSourceConfig.decryptionConfig.decryptionCode == DecryptionConfigCode.CODE_DECRYPTION_KEY) {
                        dataSourceConfig.decryptionConfig = new NEDecryptionConfig(options.dataSourceConfig.decryptionConfig.flvKey, options.dataSourceConfig.decryptionConfig.flvKeyLen);
                    }else {
